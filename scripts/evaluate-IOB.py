            ...
            if "I-" in toks_g[len(toks_g)-1]:
                ...
                if type_p == type_g:
                    ...
                    while i < len(lines_p):
                        ...
                        if line_p == "" and line_g == "":
                            break
                        elif "O" in toks_g[len(toks_g)-1]:
                            if "B-" in toks_p[len(toks_p)-1]:
                                type_p = toks_p[len(toks_p)-1].split("-")[1]
                                total_p = total_p + 1
                                total_p_d = dict_add(type_p, total_p_d)
                            break
                        elif "B-" in toks_g[len(toks_g)-1]:
                            type_g = toks_g[len(toks_g)-1].split("-")[1]
                            total_g = total_g + 1
                            total_g_d = dict_add(type_g, total_g_d) 
                            if "O" in toks_p[len(toks_p)-1]:
                ...
                if type_p == type_g:
                    ...
                    while i < len(lines_p):
                        ...
                        if line_p == "" and line_g == "":
                            ...
                            if "O" in toks_g[len(toks_g)-1]:
                                tp_inexact = tp_inexact-1
                                tp_inexact_d = dict_sub(type_p, tp_inexact_d) 
                                break
                            elif "B" in toks_g[len(toks_g)-1]:
                                i = i - 1                                                         
                                break                                                           
